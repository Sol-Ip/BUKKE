...
public class KeepPagination {
	
	// PageInfo 객체를 리턴해주는 메소드
			// static : 한번만 생성하여 정보를 조정해서 가지고 있을 수 있도록 (전역변수로 사용할 수 있도록) 하기위해 사용하여 만든 메소드
			public static KeepPageInfo getPageInfo(int currentPage, int listCount) { 
				KeepPageInfo pi = null;
				...
				pi = new KeepPageInfo(currentPage, boardLimit, pageLimit, startPage, endPage, listCount, maxPage);
				return pi;
			}
