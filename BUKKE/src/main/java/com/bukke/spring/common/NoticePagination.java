...
public class NoticePagination {
		...
		public static NoticePageInfo getPageInfo(int currentPage, int listCount) { 
			...
			int noticeLimit = 10; // 한 페이지에서 보여줄 게시글의 갯수 
			int maxPage;		// 전체 페이지 중 가장 마지막 페이지
			int startPage;		// 현재 페이지에서 시작하는 첫번째 페이지
			int endPage;		// 현재 페이지에서 끝나는 마지막 페이지
			
			maxPage = (int)((double)listCount/noticeLimit + 0.9); // listCount/boardLimt 값이 0.1로 나왔을 때 int로 변환되면 0이 되므로 0.9 해줌 
			...
			pi = new NoticePageInfo(currentPage, noticeLimit, pageLimit, startPage, endPage, listCount, maxPage);
			return pi;
		}
}
