...
public class ReviewController {
	...
	//	System.out.println(reviewLikes.getMemberId());
		...
		//	String heartYN = reviewLikes.getLikeCheck();
			...
			// 1check?좋아요 여부를 체크해줘서 스트링으로 보내준다
		    int rLikes = rService.getReviewLike(reviewNo); // 여기 다 reviewNo로 뒷부분 수정
		...
		if(review != null) {
			...
			model.addAttribute("msg", "후기 상세조회 실패");
    ...
    //    Member loginMember = (Member)session.getAttribute("loginMember");
        ...
        if(reviewLikes !=null) {
        	System.out.println("reviewLikes 현재 널아님");
        	System.out.println("reviewLikes.likesNo 은 : "+ reviewLikes.getLikesNo());
        }
        if(heartYN.equals("Y")) {
        ...
        System.out.println("heart는 :" +heartYN);
